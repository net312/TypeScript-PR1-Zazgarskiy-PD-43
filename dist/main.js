...
function validateLesson(lesson) {
  ...
  if (conflictExists) {
    console.warn(
      `Конфлікт для заняття з ID ${lesson.id} у ${lesson.dayOfWeek} з часом ${lesson.timeSlot} в аудиторії ${lesson.classroomNumber}.`,
    );
  }
...
function getClassroomUtilization(classroomNumber) {
  ...
  ).length; // Кількість занять в аудиторії
...
function reassignClassroom(lessonId, newClassroomNumber) {
  ...
  if (conflict) {
    console.warn(
      `Не вдалося змінити аудиторію для заняття з ID ${lessonId}. Конфлікт в аудиторії ${newClassroomNumber}.`,
    );
    return false; // Якщо конфлікт, повертає false
  }
  // Оновлення розкладу
  ...
  fillScheduleTable(); // Оновлення таблиці
  console.log(
    `Аудиторію для заняття з ID ${lessonId} змінено на ${newClassroomNumber}.`,
  );
  return true;
}
